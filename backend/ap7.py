...
def get_customers():
    ...
    if customer_mail:
        customers = Customer.query.filter(Customer.mail.like(f"%{customer_mail}%")).all()
    else:
...
# @app.route('/customers', methods=['GET'])
# def get_customers_by_mail_or_gender():
#     mail = request.args.get('mail')
#     gender = request.args.get('gender')
    
#     if mail:
#         customers = Customer.query.filter_by(mail=mail).all()
#     elif gender:
